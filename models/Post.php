...
//            ->whereJoin("post.user_id","candidate_education.user_id")
            ...
            ->whereJoin("candidate_education.user_id",$userId)
            ->whereJoin("post.created_at",Date::today(),">=")
            ->query("AND (post.id NOT IN (SELECT post_id FROM apply_post) OR candidate_education.user_id NOT IN (SELECT user_id FROM apply_post))")
            ->get();
    }
}